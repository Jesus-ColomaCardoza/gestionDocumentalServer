...
export class MovimientoService {
  ...
  async findOneSeguimiento2(getSeguimiento2MovimientoDto: GetSeguimiento2MovimientoDto): Promise<any> {
    try {
      type MovimientoNode = {
        Documento: {
          CreadoEl: Date
      ...
      const tramite = await this.prisma.tramite.findUnique({
        where: {
           IdTramite: getSeguimiento2MovimientoDto.IdTramite , 
      ...
      if (tramite) {
        ...
        const movimiento = tramite.Movimiento[0]


        this.message.setMessage(0, 'Trámite encontrado');
